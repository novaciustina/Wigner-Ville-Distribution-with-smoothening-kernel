...
# Lowpass the whole shifted signal beofore processing
def straighten_signal(iq_data, duration, waterfall_data):
    ...
    time = waterfall_data[:, 0]        # time points of the waterfall data, in seconds
    freq = waterfall_data[:, 1]        # frequency points of the waterfall data
...
def wigner_ville_distribution(x, max_tau):
    ...
    # A Hanning window can be applied, but it does not help with the cross-term artifacts enough to make sense
    # window = np.hanning(2 * max_tau + 1).astype(np.float32)
    # it indeed smoothens the WVD and reduces cross-term artifacts, but not enough to make it worthwhile
    # R[i] = R[i] * window   
...
#plt.imshow(wvd_db, aspect='auto', origin='lower', extent=[freqs[0], freqs[-1], time_axis[0], time_axis[-1]])
...
print(f"Average power (linear): {average_power:.3e}")
print(f"Average power (dB): {10*np.log10(average_power + 1e-20):.2f} dB")