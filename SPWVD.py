...
def wigner_ville_distribution(x, max_tau):
    ...
    # faster than the original loop-based approach
    N    = len(x)
    ...
    window = np.hanning(2 * max_tau + 1).astype(np.float32) # create a Hanning window for smoothing the WVD, shape: (2*max_tau+1,)
    window /= np.sum(window) # normalize window to preserve energy
...
# Print total energy
print(f"Total energy (linear): {total_energy:.3e}")
print(f"Total energy (dB): {10*np.log10(total_energy + 1e-20):.2f} dB")